...
class Popup {
  ...
  eventsPopup() {
    document.addEventListener("click", (function(e) {
      ...
      if (buttonClose || !e.target.closest(`[${this.options.classes.popupContent}]`) && !e.target.closest(`[data-fls-search-body]`) && this.isOpen) {
        e.preventDefault();
        this.close();
        return;
      }
...
document.querySelector(".popup__items").addEventListener("click", (e) => {
  if (e.target.closest(".item-popup__close")) {
    const item = e.target.closest(".popup__item");
    if (item) {
      e.stopPropagation();
      item.remove();
      updateTotalPrice();
...
function updateTotalPrice() {
  const items = document.querySelectorAll(".popup__item");
  let total = 0;
  items.forEach((item) => {
    const priceText = item.querySelector(".item-popup__body_price").textContent;
    const quantity = parseInt(item.querySelector("[data-fls-quantity-value]").value);
    const price = parseFloat(priceText.replace("Rs. ", "").replace(/,/g, ""));
    total += price * quantity;
  });
  document.querySelector(".popup__end-price_price").textContent = "Rs. " + total.toLocaleString();
}
function checkEmptyCart() {
  const itemsInConteiner = document.querySelector(".popup__items");
  if (itemsInConteiner.children.length === 0) {
    itemsInConteiner.innerHTML = `<div>
			Your cart is empty.
			</div>
			`;
    document.querySelector(".popup__end-price").style.display = "none";
    document.querySelector(".popup__bottom").style.display = "none";
  }
}
  ...
  document.addEventListener("click", function(e) {
    if (bodyLockStatus && e.target.closest("[data-fls-menu]")) {
      ...
      if (document.documentElement.hasAttribute("data-fls-menu-open")) {
        const menuElement2 = gsapWithCSS.timeline({ paused: true });
        menuElement2.from(".menu__item", {
          opacity: 0,
          x: -100,
          duration: 0.7,
          ease: "power2.outin",
