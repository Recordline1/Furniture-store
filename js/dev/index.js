import { c as gsapWithCSS } from "./app.min.js";
/* empty css             */
import "./cart.min.js";
class Parallax {
 ...
 * @author: Jack Doyle
 */
let gsap$4, _fonts, _coreInitted$4, _initIfNecessary = () => _coreInitted$4 || SplitText.register(window.gsap), _charSegmenter = typeof Intl !== "undefined" ? new Intl.Segmenter() : 0, _toArray$3 = (r) => typeof r === "string" ? _toArray$3(document.querySelectorAll(r)) : "length" in r ? Array.from(r) : [r], _elements = (targets) => _toArray$3(targets).filter((e) => e instanceof HTMLElement), _emptyArray$1 = [], _context$2 = function() {
  ...
  split(config) {
    ...
    if (splitCharacters || splitWords || splitLines) {
      this.elements.forEach((element, index) => {
        ...
        if (splitLines) {
          let nodes = _toArray$3(element.childNodes), wrapLine = _getLineWrapper(element, nodes, config, lines), curNode, toRemove = [], lineStartIndex = 0, allBounds = nodes.map((n) => n.nodeType === 1 ? n.getBoundingClientRect() : _emptyBounds), lastBounds = _emptyBounds;
...
*/
var gsap$3, _coreInitted$3, _win$1, _doc$1, _docEl$1, _body$1, _isTouch, _pointerType, ScrollTrigger$1, _root$1, _normalizer$1, _eventTypes, _context$1, _getGSAP$3 = function _getGSAP() {
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
var gsap$2, _coreInitted$2, _win, _doc, _docEl, _body, _root, _resizeDelay, _toArray$2, _clamp$1, _time2, _syncInterval, _refreshing, _pointerIsDown, _transformProp, _i, _prevWidth, _prevHeight, _autoRefresh, _sort, _suppressOverwrites, _ignoreResize, _normalizer, _ignoreMobileResize, _baseScreenHeight, _baseScreenWidth, _fixIOSBug, _context, _scrollRestoration, _div100vh, _100vh, _isReverted, _clampingMax, _limitCallbacks, _startup = 1, _getTime = Date.now, _time1$1 = _getTime(), _lastScrollTime = 0, _enabled = 0, _parseClamp = function _parseClamp2(value, type, self) {
  ...
  if (force === 2 || !_refreshingAll && !_isReverted) {
    ...
    var l = _triggers.length, time = _getTime(), recordVelocity = time - _time1$1 >= 50, scroll = l && _triggers[0].scroll();
  ...
  _proto.init = function init2(vars, animation) {
    ...
    if (snap) {
      ...
      _scrollers.forEach(function(o) {
        return _isFunction$1(o) && o.target === (isViewport ? _doc.scrollingElement || _docEl : scroller) && (o.smooth = false);
      });
    ...
    if (markers) {
      markerVars = _isObject$1(markers) ? _setDefaults(markers, _markerDefaults) : _markerDefaults;
...
_getGSAP$2() && gsap$2.registerPlugin(ScrollTrigger);
/*!
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
var gsap$1, _coreInitted$1, _toArray$1, _getUnit$1, _first, _ticker, _time1, _getCache$1, _getGSAP$1 = function _getGSAP3() {
  return gsap$1 || typeof window !== "undefined" && (gsap$1 = window.gsap);
}, _lookup = {}, _round$1 = function _round2(value) {
  return Math.round(value * 1e4) / 1e4;
}, _getID = function _getID2(target) {
  return _getCache$1(target).id;
}, _getByTarget = function _getByTarget2(target) {
  return _lookup[_getID(typeof target === "string" ? _toArray$1(target)[0] : target)];
}, _onTick = function _onTick2(time) {
  var pt = _first, val;
  if (time - _time1 >= 0.05) {
    _time1 = time;
    while (pt) {
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
var gsap, _coreInitted, _parseEase, _toArray, _power3, _config, _getUnit, PropTween, _getCache, _checkPointRatio, _clamp, _processingVars, _getStyleSaver, _reverting, _getTracker = VelocityTracker.getByTarget, _getGSAP4 = function _getGSAP5() {
  ...
  return typeof value === "function";
}, _bonusValidated = 1, _isArray = Array.isArray, _emptyFunc = function _emptyFunc2(p) {
  return p;
}, _bigNum = 1e10, _tinyNum = 1 / _bigNum, _checkPoint = 0.05, _round3 = function _round4(value) {
  return Math.round(value * 1e4) / 1e4;
}, _extend = function _extend2(obj, defaults, exclude) {
  for (var p in defaults) {
  ...
  return obj;
}, _deepClone = function _deepClone2(obj) {
  var copy = {}, p, v;
  for (p in obj) {
    copy[p] = _isObject2(v = obj[p]) && !_isArray(v) ? _deepClone2(v) : v;
  }
  return copy;
}, _getClosest = function _getClosest2(n, values, max, min, radius) {
...
}, _parseEnd = function _parseEnd2(curProp, end, max, min, name, radius, velocity) {
  if (curProp.end === "auto") {
    return curProp;
  }
  ...
  if (_isObject2(end)) {
    adjustedEnd = end.calculated ? end : (_isFunction2(endVar) ? endVar(end, velocity) : _getClosest(end, endVar, max, min, radius)) || end;
    if (!end.calculated) {
    ...
    adjustedEnd = _isFunction2(endVar) ? endVar(end, velocity) : _isArray(endVar) ? _getClosest(end, endVar, max, min, radius) : parseFloat(endVar);
  }
  if (adjustedEnd > max) {
    adjustedEnd = max;
...
}, _getNumOrDefault = function _getNumOrDefault2(vars, property, defaultValue) {
  return isNaN(vars[property]) ? defaultValue : +vars[property];
}, _calculateChange = function _calculateChange2(velocity, duration) {
  return duration * _checkPoint * velocity / _checkPointRatio;
}, _calculateDuration = function _calculateDuration2(start, end, velocity) {
  ...
  radius: 1,
  duration: 1
}, _processLinkedProps = function _processLinkedProps2(target, vars, getVal, resistance) {
  if (vars.linkedProps) {
    ...
    for (i = 0; i < linkedPropNames.length; i++) {
      ...
      curProp = vars[p];
      if (curProp) {
        if (_isNumber2(curProp.velocity)) {
...
}, _calculateTweenDuration = function _calculateTweenDuration2(target, vars, maxDuration, minDuration, overshootTolerance, recordEnd) {
  ...
  _isString2(target) && (target = _toArray(target)[0]);
  if (!target) {
    return 0;
  }
  var duration = 0, clippedDuration = _bigNum, inertiaVars = vars.inertia || vars, getVal = _getCache(target).get, resistance = _getNumOrDefault(inertiaVars, "resistance", _config.resistance), p, curProp, curDuration, curVelocity, curVal, end, curClippedDuration, tracker, unitFactor, linkedProps;
  linkedProps = _processLinkedProps(target, inertiaVars, getVal, resistance);
  for (p in inertiaVars) {
    if (!_reservedProps[p]) {
      curProp = inertiaVars[p];
      if (!_isObject2(curProp)) {
        tracker = tracker || _getTracker(target);
        if (tracker && tracker.isTracking(p)) {
          curProp = _isNumber2(curProp) ? {
            velocity: curProp
          } : {
...
var InertiaPlugin = {
  ...
  init: function init(target, vars, tween, index, targets) {
    ...
    var cache = target._gsap, getVal = cache.get, dur = vars.duration, durIsObj = _isObject2(dur), preventOvershoot = vars.preventOvershoot || durIsObj && dur.overshoot === 0, resistance = _getNumOrDefault(vars, "resistance", _config.resistance), duration = _isNumber2(dur) ? dur : _calculateTweenDuration(target, vars, durIsObj && dur.max || 10, durIsObj && dur.min || 0.2, durIsObj && "overshoot" in dur ? +dur.overshoot : preventOvershoot ? 0 : 1), p, curProp, curVal, unit, velocity, change1, end, change2, linkedProps;
    ...
    for (p in vars) {
      if (!_reservedProps[p]) {
        curProp = vars[p];
        _isFunction2(curProp) && (curProp = curProp(index, target, targets));
        if (_isNumber2(curProp)) {
          velocity = curProp;
        } else if (_isObject2(curProp) && !isNaN(curProp.velocity)) {
...
_getGSAP4() && gsap.registerPlugin(InertiaPlugin);
gsapWithCSS.registerPlugin(SplitText, ScrollTrigger, InertiaPlugin);
const titleDecors = gsapWithCSS.utils.toArray(".picks__title-block--title");
document.fonts.ready.then(() => {
  titleDecors.forEach((titleDecor) => {
    gsapWithCSS.from(titleDecor, {
      opacity: 0,
      y: -100,
      duration: 1,
      ease: "bounce",
      scrollTrigger: { trigger: titleDecor, start: "top 80%" }
    });
  });
});
const mainTitle = gsapWithCSS.utils.toArray(".title-with-text-decor__title");
document.fonts.ready.then(() => {
  mainTitle.forEach((title) => {
    let split = new SplitText(title, { type: "chars,words", linesClass: "line" });
    gsapWithCSS.from(split.chars, {
      scrollTrigger: {
        trigger: title,
        start: "top 80%"
        // когда заголовок войдет в зону
      },
      y: 50,
      opacity: 0,
      stagger: 0.08,
...
document.fonts.ready.then(() => {
  gsapWithCSS.from(".arrivals__image", {
    opacity: 0,
    scale: 0,
    duration: 1,
    scrollTrigger: { trigger: ".arrivals__image", start: "top 80%" }
  });
  gsapWithCSS.from([".arrivals__label", ".arrivals__title"], {
    opacity: 0,
    xPercent: 100,
    duration: 1,
    scrollTrigger: { trigger: ".arrivals__image", start: "top 80%" }
...
document.fonts.ready.then(() => {
  ...
  const instagramTitleAnim = new SplitText(instagramTitle, { type: "chars" });
  gsapWithCSS.from(instagramTitleAnim.chars, {
    scrollTrigger: {
      trigger: instagramTitle,
      start: "top 80%"
    },
    y: "random([-50,50])",
    x: "random([-100,100])",
    ...
    rotate: 360,
    duration: 1,
    onComplete: () => instagramTitleAnim.revert()
  });
});
document.fonts.ready.then(() => {
  const columns = gsapWithCSS.utils.toArray(".blogs__column");
  columns.forEach((col, i) => {
    gsapWithCSS.from(col, {
      scrollTrigger: {
        trigger: col,
        start: "top 85%"
      },
...
document.fonts.ready.then(() => {
  const elements = gsapWithCSS.utils.toArray(".advantages__column > *");
  elements.forEach((el, index) => {
    ...
    const tl = gsapWithCSS.timeline({
      scrollTrigger: {
        ...
        // или "play none none none" если не нужно повторять
      },
      onComplete: () => split.revert()
    });
    tl.from(split.lines, {
